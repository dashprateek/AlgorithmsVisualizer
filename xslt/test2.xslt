<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="create_graph" match="/SingleSourceShortestPath/graphml/graph">
        ...
        <xsl:param name="shortestPathSet" select="/SingleSourceShortestPath/iterations/shortestPath[position()&lt;=($shortestPathIndex)]"/>
    ...
    <xsl:template name = "create_node" >
        ...
        \node [<xsl:choose><xsl:when test="contains($queuedElements,concat('|',@label,'|'))">selected_node_style</xsl:when><xsl:otherwise>node_style</xsl:otherwise></xsl:choose><xsl:choose><xsl:when test="$print_label = true">,label={[text=blue]10:$<xsl:value-of select="$distance_from_source"/>$}</xsl:when></xsl:choose>](<xsl:value-of select="@id"/>) at (<xsl:value-of select="@x"/>,<xsl:value-of select="@y"/>) {<xsl:value-of select="@label"/>};
    </xsl:template>
